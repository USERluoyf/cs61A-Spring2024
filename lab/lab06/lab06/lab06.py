...
def make_change(amount, coins):
    ...
    if amount == smallest + (0 if with_smallest is None else sum(with_smallest)):
...
class ChangeMachine:
    ...
    def __init__(self, pennies):
        ...
        self.total = sum([k * v for k, v in self.coins.items()])
    def change(self, coin):
        """Return change for coin, removing the result from self.coins."""
        "*** YOUR CODE HERE ***"
        s = make_change(coin, self.coins)
        if s is None or (min(self.coins) == coin and self.coins.get(coin, None) == 1):
            return [coin]
        left_coins = self.coins_left(s)
        if left_coins:
            if sum(s) in left_coins:
                left_coins[sum(s)] += 1
                self.coins = left_coins
            else:
                self.coins = {**left_coins, **{sum(s): 1}} 
        else:
            self.coins = {sum(s): 1}
        return s
    def coins_left(self, s):
        temp = dict(self.coins)
        for coin in s:
            temp = remove_one(temp, coin)
        return temp
